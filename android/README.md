# Nivora Android

دو اپ مستقل Nivora: نسخه مشتری برای اتصال و مدیریت اشتراک، و نسخه `Nivora Partner` برای همکاران فروش.

## امکانات

- ورود، ثبت‌نام و درخواست بازیابی رمز
- نسخه مشتری فقط ورود و ثبت‌نام مشتری را دارد؛ ورود همکار در آن وجود ندارد.
- نسخه Partner فقط ورود همکار را دارد؛ ثبت‌نام همکار فقط توسط مدیریت انجام می‌شود.
- داشبورد همکار با آمار فروش، سود، کیف پول و دفترچه مشتریان
- ساخت اشتراک برای مشتری و تمدید مستقیم سرویس از داخل اپ
- ذخیره رمزگذاری‌شده توکن با Android Keystore
- اتصال و قطع VPN با `VpnService` و `libXray v26.7.28`
- پشتیبانی VLESS + Reality و شروع اعتبار از اولین اتصال
- نمایش مصرف، اعتبار، لوکیشن، تعداد دستگاه و پینگ endpoint واقعی اشتراک
- انتخاب خودکار سریع‌ترین مسیر سالم از اشتراک چندساب‌دامینی
- خرید و تمدید فوری از کیف پول و بررسی کد تخفیف
- کارت بانکی استاندارد، کپی صحیح شماره کارت و ثبت شماره پیگیری واریز
- ورود خودکار شناسه از یک پیامک جدید با تأیید خود کاربر (SMS User Consent، بدون مجوز خواندن صندوق پیامک)
- دو مسیر مستقل و امن برای تصویر رسید: دوربین و انتخاب از گالری، با سقف ۴ مگابایت
- نمایش گردش کیف پول و وضعیت درخواست‌های شارژ
- اعلان‌های حساب، ایجاد تیکت، مشاهده گفتگو و ارسال پاسخ
- تم روشن/تیره، رابط RTL و نشان مستقل Nivora بدون قاب اضافه
- رضایت شفاف حریم خصوصی پیش از اولین اتصال

## ساخت محلی

فایل AAR حجیم داخل مخزن نگهداری نمی‌شود. ابتدا هسته رسمی را دریافت کنید و بعد APK را بسازید:

```bash
bash android/fetch-libxray.sh
cd android
./gradlew app:testCustomerDebugUnitTest app:lintCustomerDebug app:assembleCustomerDebug app:assemblePartnerDebug
```

خروجی‌ها برای معماری‌های رایج جدا می‌شوند:

- `app-customer-...apk` نسخه مشتری است.
- `app-partner-...apk` نسخه مستقل همکار فروش است و هم‌زمان کنار نسخه مشتری نصب می‌شود.

API پیش‌فرض `https://b.nivorali.com` است و با Gradle property به نام `NIVORA_API_BASE_URL` قابل تغییر است.

## امضای انتشار

کلید انتشار بیرون از مخزن نگهداری می‌شود. روی رایانه اصلی، ساخت نسخه امضاشده با این دستور انجام می‌شود:

```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\android\sign-release.ps1
```

انتشار GitHub نیز فقط با Secrets رمزگذاری‌شده `NIVORA_KEYSTORE_BASE64` و `NIVORA_KEYSTORE_PASSWORD` نسخه release می‌سازد. کلید یا رمز نباید در commit، لاگ یا فایل APK عمومی قرار گیرد.

## حریم خصوصی و مجوزها

اپ کانفیگ و محتوای ترافیک را در لاگ ثبت نمی‌کند. توکن حساب با کلید سخت‌افزاری/سیستمی Android Keystore رمزگذاری و همه داده‌های اپ از Cloud Backup و انتقال دستگاه مستثنا شده‌اند. اپ مجوزهای `READ_SMS` یا `RECEIVE_SMS` درخواست نمی‌کند؛ Google Play services فقط متن یک پیامک جدید را پس از نمایش و تأیید برگه رضایت در اختیار فرم می‌گذارد. `libXray` تحت MIT و Xray-core تحت MPL-2.0 استفاده می‌شوند.
