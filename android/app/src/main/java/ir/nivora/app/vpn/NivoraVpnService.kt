...
class NivoraVpnService:VpnService(),DialerController{
 companion object{const val EXTRA_URL="subscription_url";const val ACTION_STOP="ir.nivora.app.STOP"}
 ...
 override fun onStartCommand(intent:Intent?,flags:Int,startId:Int):Int{
  ...
  thread(name="nivora-xray"){try{startTunnel(url)}catch(e:Throwable){state("error",e.message);stopTunnel(false)}}
  return START_STICKY
 }
 private fun startTunnel(url:String){stopCore();val raw=(URL(url).openConnection() as HttpURLConnection).apply{connectTimeout=15000;readTimeout=20000}.inputStream.bufferedReader().use{it.readText()}
  ...
  config.getJSONObject("env").put("xray.tun.fd",tun!!.fd)
 ...
 override fun onDestroy(){stopTunnel();super.onDestroy()};override fun onRevoke(){stopTunnel();super.onRevoke()}
}
