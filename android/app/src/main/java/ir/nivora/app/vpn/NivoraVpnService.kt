...
class NivoraVpnService : VpnService(), DialerController {
    ...
    private fun startTunnel(url: String?, shareLink: String?, sessionToken: String?, deviceId: String?, label: String, runId: Int) {
        ...
        config.put(
            "dns",
            JSONObject()
                // Resolve application names with encrypted DNS generated by
                // Xray itself. Unlike a direct port-53 rewrite, this HTTPS
                // request follows the selected proxy outbound and cannot be
                // poisoned or blocked by the access provider.
                .put("servers", JSONArray().put("https://$tunnelDns/dns-query"))
