...
class NivoraNotificationWorker(context:Context,params:WorkerParameters):Worker(context,params){
    ...
    private fun publish(items:List<CustomerNotification>){
        ...
        val manager=applicationContext.getSystemService(NotificationManager::class.java);val channel="nivora_alerts_v4";val sound=Settings.System.DEFAULT_NOTIFICATION_URI
        ...
        fresh.take(3).forEachIndexed{index,n->manager.notify(n.id.hashCode(),Notification.Builder(applicationContext,channel).setSmallIcon(R.drawable.ic_nivora_notification).setContentTitle(n.title).setContentText(n.body).setContentIntent(open).setNumber(fresh.size-index).setAutoCancel(true).build())}
    }
}
