...
import { isCustomerDeviceAuthorized, readDeviceId } from './device-bindings.js';
...
export function createSession(db, accountId, days = Number(process.env.SESSION_DAYS || 365), deviceId = null) {
  ...
  db.prepare('INSERT INTO account_sessions(id,account_id,device_id,token_hash,expires_at,created_at) VALUES(?,?,?,?,?,?)').run(randomBytes(16).toString('hex'),accountId,deviceId,tokenHash,expires.toISOString(),now.toISOString());
...
export function accountFromRequest(db, req, { requireDevice = false } = {}) {
  ...
  if (!requireDevice || !account || account.role !== 'customer') return account;
  return isCustomerDeviceAuthorized(db,account.id,readDeviceId(req)) ? account : null;
}
