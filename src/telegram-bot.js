...
export function createTelegramRecovery(db,{getConfig,fetchImpl=fetch,aiOperationsSummary,aiPublicAnswer}={}){
  ...
  return async(req,res,readJson,json)=>{
    ...
    const deleteIncoming=()=>m.message_id?fetchImpl(`https://api.telegram.org/bot${c.token}/deleteMessage`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({chat_id:chat,message_id:m.message_id})}).catch(()=>{}):Promise.resolve();
    ...
    if(m.contact){if(String(m.contact.user_id)!==user){await send(chat,'فقط شماره حساب تلگرام خودتان پذیرفته می‌شود.');return json(res,200,{ok:true});}const p=phone(m.contact.phone_number),a=db.prepare("SELECT id,name FROM accounts WHERE phone=? AND role='customer' AND status='active'").get(p);if(!a){touchLead({status:'qualified'});states.set(user,{mode:'register-name',phone:p});await send(chat,'این شماره هنوز حساب ندارد. برای ساخت حساب، نام و نام خانوادگی خود را بنویسید.',{reply_markup:{keyboard:[[{text:'لغو'}]],resize_keyboard:true}});return json(res,200,{ok:true});}db.prepare(`INSERT INTO telegram_account_links(telegram_user_id,chat_id,account_id,phone,linked_at,last_seen_at) VALUES(?,?,?,?,?,?) ON CONFLICT(telegram_user_id) DO UPDATE SET chat_id=excluded.chat_id,account_id=excluded.account_id,phone=excluded.phone,last_seen_at=excluded.last_seen_at`).run(user,chat,a.id,p,now.toISOString(),now.toISOString());touchLead({accountId:a.id,status:'linked'});await send(chat,`حساب ${a.name} با موفقیت متصل شد.`,{reply_markup:customerMenu});return json(res,200,{ok:true});}
    const registration=states.get(user);
    if(registration?.mode==='register-name'){
      if(text==='لغو'){states.delete(user);await send(chat,'ثبت‌نام لغو شد.',{reply_markup:customerMenu});return json(res,200,{ok:true});}
      const name=text.replace(/\s+/g,' ').trim();if(name.length<2||name.length>80){await send(chat,'نام باید بین ۲ تا ۸۰ کاراکتر باشد. دوباره بنویسید.');return json(res,200,{ok:true});}
      states.set(user,{...registration,mode:'register-password',name});await send(chat,'یک رمز قوی با حداقل ۸ کاراکتر بنویسید. پیام رمز پس از ثبت از گفت‌وگو حذف می‌شود.',{reply_markup:{keyboard:[[{text:'لغو'}]],resize_keyboard:true}});return json(res,200,{ok:true});
    }
    if(registration?.mode==='register-password'){
      if(text==='لغو'){states.delete(user);await send(chat,'ثبت‌نام لغو شد.',{reply_markup:customerMenu});return json(res,200,{ok:true});}
      let password;try{password=hashPassword(text)}catch{await deleteIncoming();await send(chat,'رمز باید حداقل ۸ کاراکتر باشد. رمز دیگری بنویسید.');return json(res,200,{ok:true});}
      await deleteIncoming();const id=randomUUID(),timestamp=new Date().toISOString();
      try{db.exec('BEGIN IMMEDIATE');db.prepare(`INSERT INTO accounts(id,phone,name,role,status,default_discount_percent,created_at,updated_at,password_hash,password_salt) VALUES(?,?,?,'customer','active',0,?,?,?,?)`).run(id,registration.phone,registration.name,timestamp,timestamp,password.hash,password.salt);db.prepare('INSERT INTO wallet_accounts(id,account_id,balance_toman,updated_at) VALUES(?,?,0,?)').run(randomUUID(),id,timestamp);db.prepare(`INSERT INTO telegram_account_links(telegram_user_id,chat_id,account_id,phone,linked_at,last_seen_at) VALUES(?,?,?,?,?,?) ON CONFLICT(telegram_user_id) DO UPDATE SET chat_id=excluded.chat_id,account_id=excluded.account_id,phone=excluded.phone,linked_at=excluded.linked_at,last_seen_at=excluded.last_seen_at`).run(user,chat,id,registration.phone,timestamp,timestamp);db.exec('COMMIT');}catch(error){try{db.exec('ROLLBACK')}catch{}states.delete(user);await send(chat,String(error.message||'').includes('UNIQUE')?'این شماره هم‌اکنون ثبت شده است؛ دوباره شماره را ارسال کنید.':'ساخت حساب انجام نشد؛ کمی بعد دوباره تلاش کنید.',{reply_markup:customerMenu});return json(res,200,{ok:true});}
      states.delete(user);touchLead({accountId:id,status:'linked'});log(db,actor,'register','account',id,{source:'telegram'});await send(chat,`حساب ${registration.name} با موفقیت ساخته و به تلگرام متصل شد. اکنون می‌توانید پلن را انتخاب کنید و ادامه خرید را داخل ربات یا برنامه انجام دهید.`,{reply_markup:customerMenu});return json(res,200,{ok:true});
    }
