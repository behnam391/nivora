...
export function createTelegramRecovery(db,{getConfig,fetchImpl=fetch,aiOperationsSummary,aiPublicAnswer}={}){
  ...
  return async(req,res,readJson,json)=>{
    ...
    if(m.contact){if(String(m.contact.user_id)!==user){await send(chat,'فقط شماره حساب تلگرام خودتان پذیرفته می‌شود.');return json(res,200,{ok:true});}const p=phone(m.contact.phone_number),a=db.prepare("SELECT id,name FROM accounts WHERE phone=? AND role='customer' AND status='active'").get(p);if(!a){touchLead({status:'qualified'});await send(chat,'این شماره هنوز حساب فعال Nivora ندارد؛ مشکلی نیست. می‌توانید پلن مناسب را انتخاب کنید و مراحل ثبت‌نام و خرید را ادامه دهید.',{reply_markup:{inline_keyboard:[[{text:'🧭 دریافت پیشنهاد هوشمند',callback_data:'sales:restart'}],[{text:'🛒 مشاهده همه پلن‌ها',url:storeUrl}],...(c.latestReleaseUrl?[[{text:'📥 دانلود برنامه',url:c.latestReleaseUrl}]]:[])]}});return json(res,200,{ok:true});}db.prepare(`INSERT INTO telegram_account_links(telegram_user_id,chat_id,account_id,phone,linked_at,last_seen_at) VALUES(?,?,?,?,?,?) ON CONFLICT(telegram_user_id) DO UPDATE SET chat_id=excluded.chat_id,account_id=excluded.account_id,phone=excluded.phone,last_seen_at=excluded.last_seen_at`).run(user,chat,a.id,p,now.toISOString(),now.toISOString());touchLead({accountId:a.id,status:'linked'});await send(chat,`حساب ${a.name} با موفقیت متصل شد.`,{reply_markup:customerMenu});return json(res,200,{ok:true});}
