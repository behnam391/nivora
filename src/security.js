...
export function createRequestGuard({ now = () => Date.now() } = {}) {
  ...
  const rules=[
    ...
    {test:p=>p==='/api/customer/login'||p==='/api/customer/register'||p==='/api/customer/password-reset/request'||p==='/api/customer/password-reset/confirm'||p==='/api/reseller/login',limit:10,windowMs:10*60_000},
