...
import { getWalletStatement, postWalletTransaction, postWalletTransactionInTransaction, transferWalletBalance } from './wallet.js';
...
import { createPurchaseRequests, isPurchasePath } from './purchase-requests.js';

const json = (res, status, body) => {
  if(body?.error!=='PURCHASE_PENDING')res.completePurchase?.(status, body);
...
const readJson = async (req, maxBytes = 6_000_000) => {
  if (req.purchaseBody !== undefined) return req.purchaseBody;
...
export function createApp(db, { adminToken = process.env.ADMIN_TOKEN || 'dev-only-change-me', adminUsername = process.env.ADMIN_USERNAME || '', adminPasswordSalt = process.env.ADMIN_PASSWORD_SALT || '', adminPasswordHash = process.env.ADMIN_PASSWORD_HASH || '', provisioner = null, neuralMeshManifest = null, emergencyPool: suppliedEmergencyPool = null, enforceDeviceGateway = process.env.ENFORCE_DEVICE_GATEWAY === 'true', hysteriaTicketSecret = process.env.HYSTERIA2_TICKET_SECRET || '', hysteriaTicketTtlSeconds = process.env.HYSTERIA2_TICKET_TTL_SECONDS || 45, hysteriaResumeSeconds = process.env.HYSTERIA2_RESUME_SECONDS || 43_200, hysteriaStatsMaxAgeSeconds = process.env.HYSTERIA2_STATS_MAX_AGE_SECONDS || 180, panelStatsReader = readPanelStats } = {}) {
  ...
  const purgeCustomerAccount=async({accountId,actor,resellerId=null})=>{
    const account=db.prepare("SELECT * FROM accounts WHERE id=? AND role='customer'").get(accountId);if(!account){const error=new Error('CUSTOMER_NOT_FOUND');error.status=404;throw error;}
    if(resellerId&&account.managed_by_reseller_id!==resellerId){const error=new Error('CUSTOMER_NOT_MANAGED');error.status=403;throw error;}
    const subscriptions=db.prepare(`SELECT s.id,s.panel_client_id,s.status,COALESCE(s.control_status,'active') control_status,o.id order_id,o.location_id FROM orders o JOIN subscriptions s ON s.order_id=o.id WHERE o.account_id=?`).all(account.id);
    for(const item of subscriptions){if(!item.panel_client_id||item.control_status==='deleted')continue;const location=db.prepare('SELECT * FROM service_locations WHERE id=?').get(item.location_id),remote=provisionerForLocation(location);if(!remote?.remove){const error=new Error('PANEL_CONTROL_NOT_SUPPORTED');error.status=502;throw error;}try{await remote.remove({panelClientId:item.panel_client_id});}catch(cause){const error=new Error('PANEL_CUSTOMER_DELETE_FAILED');error.status=502;error.cause=cause;throw error;}}
    const orderIds=db.prepare('SELECT id FROM orders WHERE account_id=?').all(account.id).map(row=>row.id),topupIds=db.prepare('SELECT id FROM wallet_topups WHERE account_id=?').all(account.id).map(row=>row.id),ticketIds=db.prepare('SELECT id FROM support_tickets WHERE account_id=?').all(account.id).map(row=>row.id),wallet=db.prepare('SELECT id FROM wallet_accounts WHERE account_id=?').get(account.id),receiptRows=db.prepare(`SELECT filename FROM receipt_uploads WHERE account_id=? OR (linked_entity_type='order' AND linked_entity_id IN (SELECT id FROM orders WHERE account_id=?)) OR (linked_entity_type='wallet_topup' AND linked_entity_id IN (SELECT id FROM wallet_topups WHERE account_id=?))`).all(account.id,account.id,account.id);
    const each=(items,fn)=>items.length&&fn(items.map(()=>'?').join(','),items);
    db.exec('BEGIN IMMEDIATE');try{
      each(orderIds,(marks,ids)=>{db.prepare(`UPDATE bank_transactions SET matched_order_id=NULL WHERE matched_order_id IN (${marks})`).run(...ids);db.prepare(`DELETE FROM order_reviews WHERE order_id IN (${marks})`).run(...ids);db.prepare(`DELETE FROM discount_redemptions WHERE order_id IN (${marks})`).run(...ids);db.prepare(`DELETE FROM subscriptions WHERE order_id IN (${marks})`).run(...ids);db.prepare(`UPDATE orders SET parent_order_id=NULL WHERE parent_order_id IN (${marks})`).run(...ids);});
      each(topupIds,(marks,ids)=>{db.prepare(`UPDATE bank_transactions SET matched_topup_id=NULL WHERE matched_topup_id IN (${marks})`).run(...ids);db.prepare(`DELETE FROM wallet_topup_reviews WHERE topup_id IN (${marks})`).run(...ids);});
      each(ticketIds,(marks,ids)=>db.prepare(`DELETE FROM ticket_messages WHERE ticket_id IN (${marks})`).run(...ids));
      db.prepare('DELETE FROM receipt_uploads WHERE account_id=? OR (linked_entity_type=\'order\' AND linked_entity_id IN (SELECT id FROM orders WHERE account_id=?)) OR (linked_entity_type=\'wallet_topup\' AND linked_entity_id IN (SELECT id FROM wallet_topups WHERE account_id=?))').run(account.id,account.id,account.id);
      db.prepare('DELETE FROM reseller_debts WHERE customer_account_id=?').run(account.id);db.prepare('DELETE FROM reseller_wallet_transfers WHERE customer_account_id=?').run(account.id);
      db.prepare('DELETE FROM notifications WHERE account_id=?').run(account.id);db.prepare('DELETE FROM password_reset_codes WHERE account_id=?').run(account.id);db.prepare('DELETE FROM password_reset_requests WHERE account_id=?').run(account.id);db.prepare('DELETE FROM telegram_recovery_sessions WHERE account_id=?').run(account.id);db.prepare('DELETE FROM telegram_account_links WHERE account_id=?').run(account.id);db.prepare("UPDATE sales_leads SET account_id=NULL,status='closed',updated_at=? WHERE account_id=?").run(new Date().toISOString(),account.id);
      db.prepare('DELETE FROM support_tickets WHERE account_id=?').run(account.id);db.prepare('DELETE FROM wallet_topups WHERE account_id=?').run(account.id);db.prepare('DELETE FROM orders WHERE account_id=?').run(account.id);db.prepare('DELETE FROM reseller_customers WHERE account_id=?').run(account.id);if(wallet){db.prepare('DELETE FROM wallet_transactions WHERE wallet_id=?').run(wallet.id);db.prepare('DELETE FROM wallet_accounts WHERE id=?').run(wallet.id);}db.prepare('DELETE FROM account_sessions WHERE account_id=?').run(account.id);db.prepare('DELETE FROM accounts WHERE id=?').run(account.id);db.exec('COMMIT');
    }catch(error){db.exec('ROLLBACK');throw error;}
    for(const row of receiptRows)unlink(resolve('receipts',row.filename)).catch(()=>{});audit(actor,'purge','customer_account',account.id,{phone:account.phone,orders:orderIds.length,subscriptions:subscriptions.length});return {deleted:true,orders:orderIds.length,subscriptions:subscriptions.length};
  };
  ...
  const provisionPurchaseEntries=async(plan,entries)=>{
    ...
    try{
      ...
      if(error.uncertain){
        db.prepare("UPDATE orders SET status='under_review',review_note='نتیجه پنل نامعلوم؛ پیش از بازپرداخت یا ساخت مجدد بررسی شود' WHERE id IN ("+entries.map(()=>'?').join(',')+")").run(...entries.map(entry=>entry.id));
        db.prepare("UPDATE subscriptions SET status='pending_provision',provision_error='PANEL_RESULT_UNCERTAIN' WHERE id IN ("+entries.map(()=>'?').join(',')+")").run(...entries.map(entry=>entry.subscriptionId));
  ...
  const purchaseRequests = createPurchaseRequests(db);
  const handler = async (req, res) => {
    try {
      ...
      if(req.method==='POST' && isPurchasePath(path) && req.headers['idempotency-key']) {
        const role=path.split('/')[2],account=role==='admin'?null:accountFromRequest(db,req);
        if(role==='admin'?!isAdmin(req):!account||account.role!==role)return json(res,401,{error:'UNAUTHORIZED'});
        const actor=role==='admin'?'admin':account.id,key=String(req.headers['idempotency-key']);
        req.purchaseBody=await readJson(req,32_768);
        const result=purchaseRequests.begin(actor,path,key,req.purchaseBody);
        if(result.status){if(result.replayed)res.setHeader('idempotency-replayed','true');return json(res,result.status,result.body);}
        if(!result.started){res.setHeader('retry-after','3');return json(res,409,{error:'PURCHASE_PENDING',message:'درخواست قبلی در حال پیگیری است؛ خرید دوباره ثبت نکنید.'});}
        res.completePurchase=(status,body)=>purchaseRequests.complete(actor,path,key,status,body);
      }
      ...
      if(path.startsWith('/api/customer/')){
        ...
        if(req.method==='POST'&&path==='/api/customer/connection-ready'){
          ...
          const row=db.prepare("SELECT s.id FROM subscriptions s JOIN orders o ON o.id=s.order_id WHERE o.id=? AND o.account_id=? AND o.status='approved' AND s.status='active' AND COALESCE(s.control_status,'active')='active' AND s.deleted_at IS NULL").get(String(body.orderId||''),account.id);
        ...
        if(req.method==='POST'&&path==='/api/customer/wallet/purchase'){
          ...
          try{db.exec('BEGIN IMMEDIATE');if(price>0)postWalletTransactionInTransaction(db,{accountId:account.id,amountToman:-price,type:'purchase',reference:`customer-order:${primary.id}`,actor:account.id,note:`خرید ${plan.name}${locations.length>1?` (${locations.length} لوکیشن)`:''}`});for(const entry of entries){db.prepare(`INSERT INTO orders(id,customer_name,phone,plan_id,status,amount_transferred_irr,created_at,tracking_token,account_id,location_id,bundle_id,bundle_index,bundle_size) VALUES(?,?,?,?,'approved',?,?,?,?,?,?,?,?)`).run(entry.id,account.name,account.phone,plan.id,entry.index===1?price*10:0,now,entry.trackingToken,account.id,entry.location.id,bundleId,entry.index,entries.length);db.prepare(`INSERT INTO subscriptions(id,order_id,status,access_token,created_at) VALUES(?,?,'pending_provision',?,?)`).run(entry.subscriptionId,entry.id,entry.accessToken,now);}if(discount)db.prepare('INSERT INTO discount_redemptions(id,discount_id,account_id,order_id,discount_toman,created_at) VALUES(?,?,?,?,?,?)').run(randomUUID(),discount.id,account.id,primary.id,discountToman,now);db.exec('COMMIT');}catch(error){try{db.exec('ROLLBACK')}catch{}return json(res,400,{error:error.message||'PURCHASE_FAILED'});}
          const provisioned=await provisionPurchaseEntries(plan,entries);if(provisioned.uncertain)return json(res,503,{error:'PURCHASE_PENDING',orderIds:entries.map(entry=>entry.id),refunded:false});if(!provisioned.ok){db.prepare("UPDATE orders SET status='rejected',review_note='ساخت ناموفق؛ وجه خرید کیف پول بازگردانده شد' WHERE id IN ("+entries.map(()=>'?').join(',')+")").run(...entries.map(entry=>entry.id));postWalletTransaction(db,{accountId:account.id,amountToman:price,type:'refund',reference:`customer-refund:${primary.id}`,actor:'system',note:'بازپرداخت خرید ناموفق'});if(discount)db.prepare('DELETE FROM discount_redemptions WHERE order_id=?').run(primary.id);notify(account.id,'بازپرداخت انجام شد','ساخت کامل اشتراک‌ها ناموفق بود و مبلغ به کیف پول برگشت.');return json(res,502,{error:'PROVISION_FAILED',refunded:true});}
      ...
      if(path.startsWith('/api/reseller/')){
        ...
        const resellerCustomerPurge=path.match(/^\/api\/reseller\/customers\/([^/]+)\/purge$/);
        if(req.method==='DELETE'&&resellerCustomerPurge){const body=await readJson(req);if(body.confirm!==true)return json(res,400,{error:'DELETE_CONFIRMATION_REQUIRED'});const customer=db.prepare("SELECT account_id FROM reseller_customers WHERE id=? AND reseller_id=? AND status='active'").get(resellerCustomerPurge[1],account.id);if(!customer?.account_id)return json(res,404,{error:'CUSTOMER_NOT_FOUND'});try{return json(res,200,await purgeCustomerAccount({accountId:customer.account_id,actor:account.id,resellerId:account.id}));}catch(error){return json(res,error.status||400,{error:error.message||'DELETE_FAILED'});}}
        ...
        if(req.method==='POST'&&path==='/api/reseller/purchase'){
          ...
          try{db.exec('BEGIN IMMEDIATE');if(price>0)postWalletTransactionInTransaction(db,{accountId:account.id,amountToman:-price,type:'purchase',reference:`reseller-order:${primary.id}`,actor:account.id,note:`خرید ${plan.name}${locations.length>1?` (${locations.length} لوکیشن)`:''} برای ${customer.phone}`});for(const entry of entries){db.prepare(`INSERT INTO orders(id,customer_name,phone,plan_id,status,amount_transferred_irr,created_at,tracking_token,reseller_id,account_id,location_id,reseller_customer_id,reseller_sale_price_toman,bundle_id,bundle_index,bundle_size) VALUES(?,?,?,?,'approved',?,?,?,?,?,?,?,?,?,?,?)`).run(entry.id,customer.name,customer.phone,plan.id,entry.index===1?price*10:0,now,entry.trackingToken,account.id,customer.account_id||null,entry.location.id,customer.id,entry.index===1?salePrice:0,bundleId,entry.index,entries.length);db.prepare(`INSERT INTO subscriptions(id,order_id,status,access_token,created_at) VALUES(?,?,'pending_provision',?,?)`).run(entry.subscriptionId,entry.id,entry.accessToken,now);}db.exec('COMMIT');}catch(error){try{db.exec('ROLLBACK')}catch{}return json(res,400,{error:error.message||'PURCHASE_FAILED'});}
          const provisioned=await provisionPurchaseEntries(plan,entries);if(provisioned.uncertain)return json(res,503,{error:'PURCHASE_PENDING',orderIds:entries.map(entry=>entry.id),refunded:false});if(!provisioned.ok){db.prepare("UPDATE orders SET status='rejected',review_note='ساخت ناموفق؛ وجه خرید کیف پول بازگردانده شد' WHERE id IN ("+entries.map(()=>'?').join(',')+")").run(...entries.map(entry=>entry.id));postWalletTransaction(db,{accountId:account.id,amountToman:price,type:'refund',reference:`reseller-refund:${primary.id}`,actor:'system',note:`بازپرداخت خرید ناموفق ${primary.id}`});return json(res,502,{error:'PROVISION_FAILED',refunded:true});}
      ...
      if(req.method==='POST'&&path==='/api/admin/sales'){
        ...
        if(provisioned.uncertain)return json(res,503,{error:'PURCHASE_PENDING',orderIds:entries.map(entry=>entry.id),refunded:false});if(!provisioned.ok){db.prepare("UPDATE orders SET status='rejected',review_note='فروش مستقیم ناموفق؛ اشتراک تحویل نشده است' WHERE id IN ("+entries.map(()=>'?').join(',')+")").run(...entries.map(entry=>entry.id));audit('admin','create_failed','direct_sale',primary.id,{customerId:customer.id,planId:plan.id,error:provisioned.error});return json(res,502,{error:'PROVISION_FAILED',orderIds:entries.map(entry=>entry.id)});}
      ...
      const accountPurgeMatch=path.match(/^\/api\/admin\/accounts\/([^/]+)\/purge$/);
      if(req.method==='DELETE'&&accountPurgeMatch){const body=await readJson(req);if(body.confirm!==true)return json(res,400,{error:'DELETE_CONFIRMATION_REQUIRED'});try{return json(res,200,await purgeCustomerAccount({accountId:accountPurgeMatch[1],actor:'admin'}));}catch(error){return json(res,error.status||400,{error:error.message||'DELETE_FAILED'});}}
      ...
      if(res.completePurchase){
        // An unexpected interruption must not turn into a fresh charge on retry.
        res.completePurchase=undefined;
        return json(res,503,{error:'PURCHASE_PENDING',message:'نتیجه خرید نیاز به پیگیری دارد؛ دوباره پرداخت نکنید.'});
      }
