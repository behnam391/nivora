  ...
  const handler = async (req, res) => {
    try {
      ...
      if(req.method==='POST'&&path==='/api/admin/ai/operations-summary'){
        const stats=db.prepare(`SELECT (SELECT COUNT(*) FROM accounts WHERE role='customer') customers,(SELECT COUNT(*) FROM accounts WHERE role='customer' AND status='active') active_customers,(SELECT COUNT(*) FROM accounts WHERE role='reseller' AND status='active') active_resellers,(SELECT COUNT(*) FROM orders WHERE status='under_review') pending_orders,(SELECT COUNT(*) FROM wallet_topups WHERE status='under_review') pending_topups,(SELECT COUNT(*) FROM support_tickets WHERE status<>'closed') open_tickets,(SELECT COUNT(*) FROM subscriptions WHERE status='active') active_subscriptions,(SELECT COUNT(*) FROM subscriptions WHERE status='failed' AND created_at>=datetime('now','-1 day')) failed_subscriptions_24h,(SELECT COALESCE(SUM(amount_toman),0) FROM reseller_debts WHERE status IN ('open','payment_reported')) open_debt_toman`).get();
        try{const summary=await aiCompletion({system:'شما دستیار تحلیل عملیات Nivora هستید. فقط بر اساس آمار تجمیعی داده‌شده، یک گزارش کوتاه فارسی شامل وضعیت، اولویت‌های امروز و هشدارها بنویسید. چیزی را حدس نزنید، اقدام خودکار یا وعده قطعی پیشنهاد نکنید و حداکثر ۸ خط بنویسید.',user:JSON.stringify(stats),maxTokens:500});audit('admin','analyze','ai_operations','summary',{model:aiConfig().model});return json(res,200,{summary,stats,generatedAt:new Date().toISOString()});}catch(error){return json(res,error.status||503,{error:error.message||'AI_PROVIDER_UNAVAILABLE'});}
      }
