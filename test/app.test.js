...
test('reseller wallet credits are atomic, shared-customer safe and reversible only by their owner', async t => {
  ...
  r=await fetch(`${base}/api/reseller/customers/${customer.account_id}/wallet`,{method:'POST',headers:a.auth,body:JSON.stringify({amountToman:-5000,note:'اصلاح مستقیم موجودی'})});assert.equal(r.status,200);const adjustment=await r.json();assert.equal(adjustment.balanceToman,35000);assert.equal(adjustment.resellerBalanceToman,85000);
  r=await fetch(`${base}/api/reseller/customers/${customer.account_id}/wallet`,{method:'POST',headers:a.auth,body:JSON.stringify({amountToman:-20000,note:'بیش از مانده شارژ خودم'})});assert.equal(r.status,400);assert.equal((await r.json()).error,'REVERSAL_EXCEEDS_OWN_CREDIT');
  r=await fetch(`${base}/api/reseller/customers/${customer.account_id}/wallet`,{method:'POST',headers:b.auth,body:JSON.stringify({zeroBalance:true,note:'صفر کردن غیرمجاز موجودی مشترک'})});assert.equal(r.status,400);assert.equal((await r.json()).error,'REVERSAL_EXCEEDS_OWN_CREDIT');
  r=await fetch(`${base}/api/reseller/customers`,{method:'POST',headers:a.auth,body:JSON.stringify({name:'مشتری اختصاصی',phone:'09123335555',password:'customer-pass-2'})});assert.equal(r.status,201);const exclusive=await r.json();
  r=await fetch(`${base}/api/reseller/customers/${exclusive.account_id}/wallet`,{method:'POST',headers:a.auth,body:JSON.stringify({amountToman:12000,note:'شارژ اختصاصی'})});assert.equal(r.status,201);assert.equal((await r.json()).balanceToman,12000);
  r=await fetch(`${base}/api/reseller/customers/${exclusive.account_id}/wallet`,{method:'POST',headers:a.auth,body:JSON.stringify({zeroBalance:true,note:'صفر کردن موجودی اختصاصی'})});assert.equal(r.status,200);const zeroed=await r.json();assert.equal(zeroed.balanceToman,0);
  r=await fetch(`${base}/api/reseller/customers/${exclusive.account_id}/wallet`,{method:'POST',headers:a.auth,body:JSON.stringify({zeroBalance:true,note:'موجودی از قبل صفر'})});assert.equal(r.status,200);assert.equal((await r.json()).balanceToman,0);
  ...
  r=await fetch(`${base}/api/customer/login`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({phone:'09123334444',password:'customer-pass-1'})});assert.equal(r.status,200);const customerLogin=await r.json();r=await fetch(`${base}/api/customer/me`,{headers:{authorization:`Bearer ${customerLogin.token}`}});const me=await r.json();assert.equal(me.balanceToman,35000);
  r=await fetch(`${base}/api/reseller/me`,{headers:a.auth});const resellerMe=await r.json();assert.equal(resellerMe.walletTransfers.find(item=>item.id===creditA.transferId)?.status,'partially_reversed');
});
