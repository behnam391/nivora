...
function render(){
  ...
  $('#customer-grid').innerHTML=customers.length?customers.map(customer=>{
    ...
    return `<article class="admin-customer-row"><div class="customer-primary"><span class="customer-avatar">${esc(customer.name.slice(0,1))}</span><div><h3>${esc(customer.name)}</h3><p>${esc(customer.phone)}</p></div></div><div class="customer-cell"><small>موجودی</small><b>${fa(customer.balance_toman)} تومان</b></div><div class="customer-cell"><small>دستگاه</small><b>${fa(count)} از ${fa(limit)}</b>${pending?`<em>${fa(pending)} درخواست تازه</em>`:''}</div><span class="status ${customer.status==='active'?'approved':'rejected'}">${customer.status==='active'?'فعال':'مسدود'}</span><div class="admin-customer-actions"><button class="primary customer-sale" data-id="${esc(customer.id)}">فروش اشتراک</button><button class="ghost customer-wallet" data-action="credit" data-id="${esc(customer.id)}" data-balance="${Number(customer.balance_toman)||0}">＋ افزایش</button><button class="ghost customer-wallet" data-action="debit" data-id="${esc(customer.id)}" data-balance="${Number(customer.balance_toman)||0}">− کاهش</button><button class="danger customer-wallet" data-action="zero" data-id="${esc(customer.id)}" data-balance="${Number(customer.balance_toman)||0}">صفر کردن</button><button class="ghost customer-edit" data-id="${esc(customer.id)}">ویرایش</button><button class="${pending?'primary':'ghost'} customer-devices" data-id="${esc(customer.id)}">دستگاه‌ها${pending?' · جدید':''}</button><button class="danger customer-purge" data-id="${esc(customer.id)}">حذف کامل</button></div></article>`;
  ...
  document.querySelectorAll('.customer-purge').forEach(button=>button.onclick=()=>purgeCustomer(button.dataset.id));
...
async function purgeCustomer(id){
  ...
  if(!await adminConfirm({title:'حذف کامل مشتری',message:`حساب ${customer.name}، اشتراک‌های سرورها، کیف پول، سفارش‌ها، رسیدها، تیکت‌ها و اتصال تلگرام برای همیشه حذف می‌شوند. این عملیات قابل بازگشت نیست.`,confirmText:'حذف کامل و قطعی',danger:true}))return;
  try{await api(`/api/admin/accounts/${encodeURIComponent(id)}/purge`,{method:'DELETE',body:JSON.stringify({confirm:true})});toast('کاربر و تمام اطلاعات وابسته حذف شدند');await loadCustomers()}
  catch(error){toast(error.message==='PANEL_CUSTOMER_DELETE_FAILED'?'حذف از پنل سرور انجام نشد؛ حساب حذف نشد.':'حذف کامل کاربر انجام نشد.',true)}
}
