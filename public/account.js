...
async function api(url,options={}){
  const response=await window.NivoraPurchaseFetch(url,{...options,headers:{...(options.headers||{}),...(token?{authorization:`Bearer ${token}`}:{})}});
...
function render(account){
  ...
  $('#orders').innerHTML=account.orders.length?account.orders.map(order=>{const [label,state]=statusInfo(order);const remaining=order.duration_days!==0&&Number.isFinite(Number(order.remainingDays))?` · ${fa(order.remainingDays)} روز مانده`:'';const canConnect=order.subscription_status==='active'&&order.control_status==='active';return `<article><div><h3>${esc(order.plan_name)}</h3><p>${esc(order.special_message||'')}</p><small>${order.order_kind==='renewal'?'تمدید':'خرید'} · ${order.traffic_gb===0?'حجم نامحدود':fa(order.traffic_gb)+' گیگ'} · ${order.duration_days===0?'زمان نامحدود':fa(order.duration_days)+' روز'} · ${fa(order.device_limit)} دستگاه${remaining}</small><span class="status-pill ${state}">${esc(label)}</span></div><div class="actions">${canConnect?'<span class="app-guidance">اندروید: اپ Nivora · آیفون: V2Box</span>':''}${canConnect?`<button class="ios-connect-button" data-ios="${esc(order.id)}">اتصال آیفون</button>`:''}${order.order_kind==='purchase'&&canConnect?`<button data-renew="${esc(order.id)}">تمدید</button>`:''}</div></article>`}).join(''):empty('هنوز اشتراکی ندارید.','برای شروع یک پلن انتخاب کنید.');
...
async function prepareIosImport(button){
  ...
  try{
    ...
    const deepLink=`v2box://install-sub?url=${encodeURIComponent(subscription.href)}&name=${encodeURIComponent(result.profileName||'Nivora')}`;
    const open=$('#ios-open');open.href=deepLink;open.classList.remove('disabled');open.setAttribute('aria-disabled','false');
    $('#ios-status').textContent='اتصال آماده است؛ اکنون دکمه آبی را لمس و افزودن پروفایل را در V2Box تأیید کنید.';
...
function fillPlans(){
  ...
  const show=()=>{const plan=plans.find(item=>item.id===select.value);$('#plan-info').textContent=plan?`${plan.trafficGb===0?'حجم نامحدود':fa(plan.trafficGb)+' گیگ'} · ${plan.durationDays===0?'زمان نامحدود':fa(plan.durationDays)+' روز'} · ${fa(plan.deviceLimit)} دستگاه · ${fa(plan.priceIrr)} تومان`:''};
  select.onchange=()=>{show();const plan=plans.find(item=>item.id===select.value);$('#plan-info').insertAdjacentHTML('beforeend',`<div>${countryBadges(plan?.locations)} ${esc(plan?.specialMessage||'')}</div>`)};select.onchange();
}
